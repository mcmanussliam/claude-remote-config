...
addCommonOptions(program.command('sync')).action(async (options) => {
    await run(async () => {
        const result = await initProject({
            projectDir: options.project ?? process.cwd(),
            pluginDataDir: options.pluginData ?? DEFAULT_PLUGIN_DATA_DIR,
...
addCommonOptions(program.command('explain')).action(async (options) => {
    await run(async () => {
        const result = await initProject({
            projectDir: options.project ?? process.cwd(),
            pluginDataDir: options.pluginData ?? DEFAULT_PLUGIN_DATA_DIR,
...
addCommonOptions(program.command('print')).action(async (options) => {
    await run(async () => {
        const result = await initProject({
            projectDir: options.project ?? process.cwd(),
            pluginDataDir: options.pluginData ?? DEFAULT_PLUGIN_DATA_DIR,
...
async function run(fn, hookMode = false) {
    try {
        await fn();
    }
    catch (error) {
